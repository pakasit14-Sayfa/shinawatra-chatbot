...
const ChatContent = (props: Props) => {
  ...
  // Handoff Toggle Function
  const toggleHandoff = async (locked: boolean) => {
    try {
      await fetch('/api/chat/toggle', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: activeUser?.id, locked })
      })
      alert(locked ? 'Takeover สำเร็จ: บอทหยุดตอบแล้ว' : 'เปิดบอทสำเร็จ: บอทกลับมาทำงานแล้ว')
    } catch (e) {
      console.error(e)
    }
  }
  ...
  return !chatStore.activeUser ? (
    ...
    <>
      {activeUser && (
        <div className='flex flex-col grow bs-full bg-backgroundChat'>
